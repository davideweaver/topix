...
describe('ConfigManager', () => {
  ...
  const testDbPath = join(__dirname, 'test-config-manager.db');
  ...
  describe('Feed Configuration', () => {
    ...
    it('should set feed config', () => {
      ...
      expect(feedConfig.description).toBe('Curated headlines from your digital life'); // Unchanged
    });
  });
  ...
  describe('Cache Management', () => {
    it('should clear cache', () => {
      // Get preferences to populate cache
      const prefs1 = configManager.getPreferences();

      // Clear cache
      configManager.clearCache();

      // Get preferences again (should load from database)
      const prefs2 = configManager.getPreferences();
